...
def getHeader(string, name):
    ...
    headerdict = {}
    headers = bytes.decode(string).strip('\r').split('\r\n')
    for header_str in headers:
        split_point = header_str.find(':')
        if split_point == -1:
            continue
        key = header_str[:split_point].strip()
        val = header_str[split_point + 1:].strip()
        headerdict[key.upper()] = val

    print(headerdict)
    return headerdict.get(name,None)
...
def thread_proxy(client, addr):
    ...
    print(f'responseHeader:{responseHeader}')
    # 响应头过长！
